...
export class EnterpriseApi {
  ...
  async init(currentDirectory: string, command?: Command) {
    ...
    if (!gardenEnv.GARDEN_AUTH_TOKEN && commandAllowed) {
  ...
  async refreshToken() {
    ...
    try {
      ...
      if (res && res.statusCode === 401) {
        ...
        // await clearAuthToken(this.log)
      } else {
        throw new RuntimeError(
          `Generic = An error occurred while verifying client auth token with platform: ${err.message}`,
          {}
        )
      }
    }
  ...
  async readAuthToken(): Promise<string | null> {
    ...
    this.log.debug(`Retrieved client auth token from local config db`)
